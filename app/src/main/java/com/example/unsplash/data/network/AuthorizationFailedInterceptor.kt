...
class AuthorizationFailedInterceptor(
    ...
    private fun refreshToken(): Boolean {
        ...
        val tokenRefreshed = runBlocking {
            runCatching {
                val refreshRequest = authRepository.getRefreshTokenRequest(
                    tokenStorage.refreshToken.orEmpty()
                )
                authRepository.performTokenRequest(authorizationService, refreshRequest)
...
//            unauthorizedHandler.onUnauthorized()
            Log.d(TAG_AUTH, "AuthorizationFailedInterceptor: logout after token refresh failure")
        }
        getLatch()?.countDown()
